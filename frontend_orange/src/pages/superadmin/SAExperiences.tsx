              ...
              {experiences.map((exp: any) => {
                ...
                return (
                  <tr key={exp._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ fontSize: 11, color: 'rgba(240,237,232,0.5)' }}>{cat?.emoji} {cat?.label || exp.category}</span>
                    </td>
...
function ExperienceFormModal({ exp, onClose }: { exp: any; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div style={{ background: '#0f0f1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 640, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ color: '#f0ede8', fontSize: 18, fontWeight: 700 }}>{isEdit ? 'Modifica' : 'Nuova'} esperienza</h2>
          <button onClick={onClose} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(255,255,255,0.05)', color: 'rgba(240,237,232,0.5)' }}><X size={15} /></button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 6, background: 'rgba(255,255,255,0.04)', padding: 3, borderRadius: 8 }}>
              {(['url', 'file'] as const).map(m => (
                <button key={m} onClick={() => setImgMode(m)} style={{ flex: 1, padding: '5px', borderRadius: 6, border: 'none', cursor: 'pointer', background: imgMode === m ? 'rgba(232,98,42,0.2)' : 'transparent', color: imgMode === m ? '#e8622a' : 'rgba(240,237,232,0.4)', fontSize: 11, fontWeight: 600 }}>
            ...
            {imgMode === 'url' ? (
              ...
              <>
                ...
                <button onClick={() => fileRef.current?.click()} disabled={uploading} style={{ width: '100%', padding: '10px', borderRadius: 10, border: '2px dashed rgba(232,98,42,0.3)', background: 'rgba(232,98,42,0.05)', color: 'rgba(240,237,232,0.5)', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            {[
              ...
              <label key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: 'rgba(240,237,232,0.6)' }}>
                <input type="checkbox" checked={form[k]} onChange={e => set(k, e.target.checked)} style={{ accentColor: '#e8622a', width: 15, height: 15 }} />
                {label}
              </label>
            ))}
          </div>
          ...
          <div>
            ...
            <div style={{ position: 'relative', marginBottom: 10 }}>
              <Search size={13} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'rgba(240,237,232,0.3)' }} />
              ...
              {stopSearch.length > 1 && placesData?.data?.length > 0 && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 10, background: '#1a1a2e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, marginTop: 4, overflow: 'hidden' }}>
                  {placesData.data.map((place: any) => (
                    <button key={place._id} onClick={() => addStop(place)} style={{ width: '100%', padding: '8px 12px', border: 'none', cursor: 'pointer', background: 'transparent', color: '#f0ede8', fontSize: 13, textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, transition: 'background 0.15s' }}
                      onMouseEnter={e => (e.currentTarget.style.background = 'rgba(232,98,42,0.1)')}
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 10, color: 'rgba(240,237,232,0.4)' }}>{place.location?.neighborhood}</p>
                      </div>
                      <Plus size={12} style={{ marginLeft: 'auto', color: '#e8622a' }} />
                    </button>
                  ))}
                </div>
              )}
            ...
            {/* Stops list */}
            {form.stops.length === 0 ? (
              <div style={{ padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, textAlign: 'center', color: 'rgba(240,237,232,0.3)', fontSize: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
            ...
            <button onClick={() => mutation.mutate()} disabled={!form.title || mutation.isPending}
              style={{ flex: 2, padding: '11px', borderRadius: 12, border: 'none', background: form.title ? 'linear-gradient(135deg,#e8622a,#f0884a)' : 'rgba(255,255,255,0.08)', color: form.title ? '#fff' : 'rgba(240,237,232,0.3)', cursor: form.title ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 700 }}>
