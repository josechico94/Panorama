...
function BarChart({ data, color = '#e8622a' }: { data: number[]; color?: string }) {
...
function DonutChart({ segments }: { segments: { value: number; color: string; label: string }[] }) {
  ...
  if (total === 0) return <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: 10, color: 'rgba(240,237,232,0.3)' }}>—</span></div>
...
const C = {
  card: { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: 20 } as React.CSSProperties,
  label: { fontSize: 10, fontWeight: 700, color: 'rgba(240,237,232,0.35)', textTransform: 'uppercase', letterSpacing: '0.12em' } as React.CSSProperties,
}
...
export default function SADashboard() {
  ...
  const statCards = [
    { label: 'Luoghi totali', value: s?.places ?? 0, icon: MapPin, color: '#e8622a', sub: 'in piattaforma' },
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => refetch()} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'rgba(240,237,232,0.6)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
        ...
        <div style={C.card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 22, fontWeight: 800, color: '#f0ede8', marginTop: 2 }}>{s?.users ?? 0}</p>
            </div>
        ...
        <div style={C.card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 22, fontWeight: 800, color: '#f0ede8', marginTop: 2 }}>{s?.userCoupons ?? 0}</p>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16 }}>
        ...
        <div style={C.card}>
          ...
          {(s?.recentUsers ?? []).length === 0 ? (
            <p style={{ color: 'rgba(240,237,232,0.3)', fontSize: 12, textAlign: 'center', padding: '16px 0' }}>Nessun utente</p>
          ) : (
        ...
        <div style={C.card}>
          ...
          {(s?.topPlaces ?? []).length === 0 ? (
            <p style={{ color: 'rgba(240,237,232,0.3)', fontSize: 12, textAlign: 'center', padding: '16px 0' }}>Nessun dato</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div key={p._id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ height: 3, background: 'rgba(255,255,255,0.05)', borderRadius: 2, marginTop: 4, overflow: 'hidden' }}>
                      <div style={{ height: '100%', background: '#e8622a', borderRadius: 2, width: `${Math.min(100, ((p.meta?.views ?? 0) / Math.max(...(s?.topPlaces ?? []).map((x: any) => x.meta?.views ?? 1), 1)) * 100)}%`, transition: 'width 0.5s' }} />
                    </div>
                  </div>
        ...
        <div style={C.card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { label: 'Coupon creati', value: s?.coupons ?? 0, color: '#e8622a', max: Math.max(s?.coupons ?? 1, 1) },
