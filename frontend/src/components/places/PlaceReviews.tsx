      ...
      {[1,2,3,4,5].map(i => (
        ...
        >
          <Star
            ...
            color={(hover || rating) >= i ? '#f59e0b' : 'var(--border)'}
    ...
    <div>
      ...
      {!showForm && (
        <button
          ...
          style={{
            ...
            background: 'rgba(245,158,11,0.05)', color: 'var(--text-3)',
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <p style={{ color: 'var(--text-2)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 10 }}>
              La tua recensione
            </p>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setShowForm(false)} style={{ flex: 1, padding: '9px', borderRadius: 10, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-3)', cursor: 'pointer', fontSize: 12 }}>
                Annulla
              </button>
              <button
                onClick={() => createMutation.mutate()}
                disabled={rating === 0 || createMutation.isPending}
                style={{ flex: 2, padding: '9px', borderRadius: 10, border: 'none', background: rating > 0 ? '#f59e0b' : 'rgba(255,255,255,0.08)', color: rating > 0 ? '#000' : 'var(--text-3)', cursor: rating > 0 ? 'pointer' : 'not-allowed', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}
              >
            ...
            <div key={r._id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ color: 'var(--text-3)', fontSize: 10, fontFamily: 'DM Mono,monospace' }}>
                    {new Date(r.createdAt).toLocaleDateString('it-IT')}
                  </span>
                  {isOwn && (
                    <button onClick={() => deleteMutation.mutate(r._id)}
                      style={{ padding: 4, borderRadius: 6, border: 'none', cursor: 'pointer', background: 'transparent', color: 'var(--text-3)' }}
