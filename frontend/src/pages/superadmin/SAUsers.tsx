...
const field = { width: '100%', padding: '10px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#f0ede8', fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'rgba(240,237,232,0.35)', fontSize: 12, marginTop: 2 }}>{data?.total ?? 0} registrati</p>
      </div>
                ...
                <tr key={user._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                    <button onClick={() => confirm(`Eliminare utente "${user.name}"?`) && deleteMutation.mutate(user._id)}
                      style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'transparent', color: 'rgba(240,237,232,0.3)', transition: 'all 0.15s' }}
