...
// ── Categorie locali in localStorage (frontend-only per ora) ──
// Per persistenza backend si può aggiungere un endpoint /superadmin/categories
const STORAGE_KEY = 'faf-custom-categories'

const DEFAULT_CATEGORIES = [
  { id: 'eat',     label: 'Mangiare',    emoji: '🍽️', color: '#f97316' },
  { id: 'drink',   label: 'Bere',        emoji: '🍹', color: '#a855f7' },
  { id: 'shop',    label: 'Shopping',    emoji: '🛍️', color: '#ec4899' },
  { id: 'walk',    label: 'Passeggio',   emoji: '🚶', color: '#22c55e' },
  { id: 'culture', label: 'Cultura',     emoji: '🏛️', color: '#3b82f6' },
  { id: 'sport',   label: 'Sport',       emoji: '⚡',  color: '#84cc16' },
  { id: 'night',   label: 'Notte',       emoji: '🌙', color: '#6366f1' },
]
...
export default function SACategories() {
  ...
  const handleSave = () => {
    ...
    if (!form.id.trim()) { setError('L\'ID è obbligatorio'); return }
    if (!editCat && categories.find((c: any) => c.id === form.id)) { setError('ID già esistente'); return }

    let updated
    if (editCat) {
      updated = categories.map((c: any) => c.id === editCat.id ? { ...c, ...form } : c)
    } else {
  ...
  const handleDelete = (id: string) => {
    if (DEFAULT_CATEGORIES.find(c => c.id === id)) {
      alert('Non puoi eliminare le categorie di default. Puoi modificarle.')
      return
    }
  ...
  const handleReset = () => {
    if (!confirm('Ripristinare le categorie di default? Perderai le categorie custom.')) return
    setCategories(DEFAULT_CATEGORIES)
    saveCategories(DEFAULT_CATEGORIES)
  }

  const generateId = (label: string) =>
    label.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'rgba(240,237,232,0.4)', fontSize: 12, marginTop: 2 }}>{categories.length} categorie · usate in tutti i luoghi</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleReset} style={{ padding: '9px 16px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'rgba(240,237,232,0.4)', cursor: 'pointer', fontSize: 12 }}>
            Reset default
          </button>
      ...
      <div style={{ padding: '12px 16px', background: 'rgba(187,0,255,0.08)', border: '1px solid rgba(187,0,255,0.2)', borderRadius: 14, marginBottom: 20 }}>
        <p style={{ fontSize: 12, color: 'rgba(187,0,255,0.9)', lineHeight: 1.5 }}>
          💡 Le categorie vengono usate per classificare i luoghi. L'ID deve essere univoco e senza spazi (es: <code style={{ fontFamily: 'DM Mono', background: 'rgba(187,0,255,0.15)', padding: '1px 5px', borderRadius: 4 }}>benessere</code>). Le modifiche si applicano subito a tutti i luoghi.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
          ...
          return (
            <motion.div
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow = `0 4px 16px ${cat.color}20`
              }}
              onMouseLeave={e => {
              ...
              <div style={{ padding: '16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ color: '#f0ede8', fontSize: 14, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {cat.label}
                    </p>
                    <p style={{ color: 'rgba(240,237,232,0.35)', fontSize: 10, fontFamily: 'DM Mono', marginTop: 2 }}>
                      id: {cat.id}
                    </p>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  {isDefault && (
                    <span style={{ fontSize: 9, color: 'rgba(240,237,232,0.3)', background: 'rgba(255,255,255,0.05)', padding: '2px 7px', borderRadius: 100, fontWeight: 600 }}>
                      DEFAULT
                    </span>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  {!isDefault && (
                    <button onClick={() => handleDelete(cat.id)} style={{ width: 32, padding: '7px', borderRadius: 9, border: '1px solid rgba(248,113,113,0.2)', background: 'rgba(248,113,113,0.06)', color: '#f87171', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Trash2 size={11} />
                    </button>
                  )}
                </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={openNew}
          style={{ background: 'transparent', border: '2px dashed rgba(187,0,255,0.3)', borderRadius: 16, padding: '32px 16px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, transition: 'all 0.2s' }}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: `${form.color}12`, border: `1px solid ${form.color}30`, borderRadius: 14, marginBottom: 20 }}>
                <div style={{ width: 48, height: 48, borderRadius: 13, background: `${form.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
                  {form.emoji}
                </div>
                <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setForm(f => ({ ...f, label, id: editCat ? f.id : generateId(label) }))
                    }}
                    placeholder="Es: Benessere"
                    style={field}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="benessere"
                    disabled={!!editCat}
                    style={{ ...field, opacity: editCat ? 0.5 : 1, fontFamily: 'DM Mono,monospace' }}
                    onFocus={e => (e.target.style.borderColor = '#BB00FF')}
                    onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.1)')}
                  />
                  {editCat && <p style={{ fontSize: 10, color: 'rgba(240,237,232,0.3)', marginTop: 4 }}>L'ID non può essere modificato dopo la creazione</p>}
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {EMOJI_PRESETS.map(e => (
                      <button key={e} onClick={() => setForm(f => ({ ...f, emoji: e }))} style={{ width: 36, height: 36, borderRadius: 8, border: `2px solid ${form.emoji === e ? '#BB00FF' : 'transparent'}`, background: form.emoji === e ? 'rgba(187,0,255,0.15)' : 'rgba(255,255,255,0.05)', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
                        {e}
                      </button>
                    ))}
                    <input
                      ...
                      placeholder="o scrivi"
                      style={{ ...field, width: 80, textAlign: 'center', fontSize: 18 }}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                    {COLOR_PRESETS.map(color => (
                      <button key={color} onClick={() => setForm(f => ({ ...f, color }))} style={{ width: 32, height: 32, borderRadius: 8, border: `3px solid ${form.color === color ? '#fff' : 'transparent'}`, background: color, cursor: 'pointer', transition: 'transform 0.15s', transform: form.color === color ? 'scale(1.15)' : 'scale(1)' }} />
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
                  ...
                  <button onClick={handleSave} style={{ flex: 2, padding: '11px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#BB00FF,#9000CC)', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>
                    {editCat ? '✓ Salva modifiche' : '✓ Crea categoria'}
                  </button>
                </div>
              </div>
            </motion.div>
