...
function CouponEditModal({ coupon, onClose }: { coupon: any; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div style={{ background: '#0f0f1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ color: 'rgba(240,237,232,0.35)', fontSize: 12, marginBottom: 20 }}>
          Locale: <span style={{ color: '#e8622a' }}>{(coupon.placeId as any)?.name || '—'}</span>
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
              {[{ id: 'percentage', label: '% Percentuale' }, { id: 'fixed', label: '€ Fisso' }, { id: 'freebie', label: '🎁 Omaggio' }].map(({ id, label }) => (
                <button key={id} onClick={() => set('discountType', id)} style={{
                  padding: '9px 0', borderRadius: 10, border: `1px solid ${form.discountType === id ? '#e8622a' : 'rgba(255,255,255,0.1)'}`,
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: 'rgba(240,237,232,0.6)' }}>
            <input type="checkbox" checked={form.active} onChange={e => set('active', e.target.checked)} style={{ accentColor: '#e8622a', width: 15, height: 15 }} />
            ✅ Coupon attivo
          </label>
          ...
          <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
            ...
            <button onClick={() => mutation.mutate()} disabled={!form.title || mutation.isPending}
              style={{ flex: 2, padding: '11px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#e8622a,#f0884a)', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 700, opacity: mutation.isPending ? 0.7 : 1 }}>
